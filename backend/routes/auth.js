...
router.get("/getUserData", async (req, res) => {
  ...
  console.log(email, "fdf");

  try {
    const user = await Compilation.findOne({ email });
    if (!user) {
...
router.post("/storeCompilationData", async (req, res) => {
  const { username, email, language, difficulty, code, result, success } =
    req.body;
  ...
  try {
    ...
    let user = await Compilation.findOne({ email });

    if (!user) {
      // If the user does not exist, create a new user
      user = new Compilation({
        name: username,
        email,
        totalPoints: 0,
        compilationHistory: [],
    ...
    // Update the user's total points and add to compilation history
    user.totalPoints += points;
    user.compilationHistory.push(newCompilation);

    // Save the user document
    await user.save();

    res.status(200).send({ message: "Compilation data stored successfully" });
  } catch (error) {
    res
      .status(500)
      .send({ message: "Failed to store compilation data", error });
  }
});
